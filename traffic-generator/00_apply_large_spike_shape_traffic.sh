#!/bin/bash
if [ $# -lt 1 ]; then
  echo "Usage: apply_large_spike_shape_traffic.sh <namespace>"
  exit 1
fi
...
kubectl -n $1 create configmap acme-locustfile-shaped --from-file=locustfile.py=./locustfile.py --from-file=shape.py=./shape_large_spike.py
